...
public class ID3TagEditor {
     ...
     */
    public func read(from path: URL) throws -> ID3Tag? {
     ...
     **If nil, the mp3 file will be overwritten**.
     ...
     ID3 tag).
     */
    public func write(tag: ID3Tag, to path: URL, andSaveTo newPath: String? = nil) throws {
        ...
        try mp3WithID3TagBuilder.buildMp3WithNewTagAndSaveUsing(
                ...
                andSaveTo: newPath ?? path.absoluteString
        )
    }

    private func readMp3From(path: URL) throws -> Data {
        guard path.pathExtension.caseInsensitiveCompare("mp3") == ComparisonResult.orderedSame else {
            throw ID3TagEditorError.InvalidFileFormat;
        }
        let mp3 = try Data(contentsOf: path)
        return mp3
    }
}
